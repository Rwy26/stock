...
TP_SANITY_MAX_AGE_DAYS = 365  # 이보다 오래된 리포트는 현재가 sanity 미적용(노이즈 방지)
    ...
    for idx, (code, master_name) in enumerate(uni, 1):
        ...
        try:
            ...
            for row in rows:
                ...
                # sanity 는 '현재가' 대비라 최근 리포트에만 의미 있다(옛 리포트의 목표가를
                # 오늘 현재가와 비교하면 분할·장기등락으로 거의 다 '비정상'이 됨 → 노이즈).
                recent = (date.today() - row["report_date"]).days <= TP_SANITY_MAX_AGE_DAYS
                if tp and cur_price and cur_price > 0 and recent:
                    ratio = tp / cur_price
...
LOCK_FILE = REPO / "logs" / "research-reports-sync.lock"


def _acquire_lock():
    """단일 인스턴스 락(Windows msvcrt). 다른 실행이 점유 중이면 None 반환.

    예약작업(IgnoreNew) + 수동/타 세션 동시 실행이 같은 로그에 append → 손상·네이버 중복부하
    겪은 뒤 도입. 락 못 잡으면 조용히 종료(중복 수집 방지)."""
    try:
        import msvcrt
        ...
        return "noop"  # msvcrt 미가용 환경: 락 없이 진행(비-Windows 안전장치)
...
def main() -> int:
    ...
    lock = _acquire_lock()
    if lock is None:
        log("SKIP: 다른 research-reports-sync 인스턴스 실행 중 — 중복 수집 방지로 종료")
        return 0
