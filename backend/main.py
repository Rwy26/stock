    ...
    def _postprocess(result: dict) -> None:
        ...
        if verified and apollo_db is not None and models is not None:
            try:
                ...
                pass
            # 관심종목 자동 편입 + 12단계 분석 → 일일 배치가 매일 갱신 (관리자 이미지 분석)
            result["induction"] = _induct_verified_stock(vcode, vname)
...
def ai_crossval_analyze(symbol: str, _current_user=Depends(require_admin)):
    ...
    if isinstance(result, dict):
        ...
        if verified:
            ...
            # ── 관심종목 자동 편입 + 12단계 분석 → 일일 배치가 매일 갱신 ──
            result["induction"] = _induct_verified_stock(vcode, vname)
        else:
            result["identityNote"] = (
...
def _induct_verified_stock(code: str, name: str | None) -> dict:
    """검증된 종목을 [DB 등록 → 제외게이트 → 관심종목 편입 → 12단계 분석] 파이프라인에 투입.

    관심종목에 편입되면 일일 배치(21:00 batch_analyze)가 매일 자동 업데이트한다.
    반환: {inducted, alreadyHad, excluded}.
    """
    out = {"inducted": False, "alreadyHad": False, "excluded": False}
    if apollo_db is None or models is None or not code:
        return out
    try:
        import exclusion_engine
...
5. 한국어를 우선으로 쓴다. 쉽고 일반적인 영어(SELL·BUY·HOLD·AI·ETF·RSI·VWAP·SK·LG 등)는 설명 없이 그대로 쓴다.
   전문·생소한 영어 용어만 바로 뒤에 한국어 설명을 괄호로 병기한다 — 예: ADAS(첨단 운전자 지원 시스템), CHoCH(추세 전환), FVG(미충전 갭).
6. 모든 수치는 반드시 아라비아 숫자로 표기한다 — 한글 숫자(예: '오십삼 점 칠', '삼십육만 삼천')는 절대 쓰지 않는다.
   예) 종합점수 53.7, 현재가 363,000원, 평균목표가 468,582원, 상승여력 29.1%, 손익비 2.85.
   가격은 천 단위 콤마, 퍼센트는 % 기호, 소수점은 숫자로 표기한다.
7. 한국어·영어 외 문자(한자(漢字)·중국어·일본어 등)는 절대 쓰지 않는다.
...
def public_ai_search(request: Request, body: dict = Body(default={})):
    """공개 자연어 대화형 검색 — 게스트 게이트 (대화 횟수 제한 없음). 검색 기록은 관리자에 저장."""
    try:
    ...
    g_name = str(body.get("guest_name") or "").strip()
    g_phone = str(body.get("guest_phone") or "").strip()
    if not (g_name and g_phone):
        raise HTTPException(status_code=403, detail="이름과 전화번호가 필요합니다 (게스트 입장 후 이용).")

    # 게스트 검색 기록 저장 → 관리자 '공개 요청' 화면에서 확인 (대화 제한은 없음)
    try:
        if apollo_db is not None and models is not None:
            with apollo_db.session_scope() as s:
                s.add(models.PublicAiRequest(
                    name=_clean_public_text(g_name), phone=_clean_public_text(g_phone),
                    stock_query=f"[검색] {query[:120]}"))
    except Exception:
        pass
